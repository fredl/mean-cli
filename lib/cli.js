...
function getUserCollectionName(callback) {
      var cfg = require(process.cwd() + '/server/config/env/all.js');
      var usrColl = cfg.userCollection || 'users';

      if (usrColl === 'users') {
        console.info('    To specify user collection,'.info);
        console.info('    please add an entry to env/all.js'.info);
        console.info('         - userCollection: \'userCollectionName\''.warn);
      };
      callback(usrColl);
}
...
exports.printUser = function(email, options) {
  requiresRoot(function() {
    mongoConnect(options.env, function(err, db) {
      ...
      getUserCollectionName(function(name){
        var usrColl = name;
        console.info('\nCurrent User Collection name : "' + usrColl + '"');
        db.collection(usrColl).find({
          email: email
